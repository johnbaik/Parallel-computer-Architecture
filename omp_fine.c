...
int main(int argc, char**args){
    ...
    //printf("name=%s ,path=%s match=%d, missmatch=%d, gap=%d",name,path,MATCH,MISSMATCH,GAP);
...
void ScoreTable(char seq1[],char seq2 []){
    ...
    int seq2len = strlen(seq2);
     ...
     int cells=0;
     int ID=0;
     int wave, tStart,tEnd,nothreads;
 ...
 #pragma omp parallel shared(seq1,seq2,M)  private(i,j,wave,tStart,tEnd,ID,cells,scoreDiag,scoreLeft,scoreUp,maxScore) num_threads(threads)
     {
         ...
         tStart= (((seq2len/nothreads)*ID)+1);
         
         
         tEnd= tStart+(seq2len/nothreads);
    ...
    #pragma omp barrier
                    ...
                    double st =gettime();
                    count_cells=count_cells+(seq1len*seq2len);
                    cells_calc_time=cells_calc_time+(st-s);
}
